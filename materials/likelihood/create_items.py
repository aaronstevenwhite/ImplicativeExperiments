...
follow_up_test = lambda pred_tuple: 'How likely is it that {} {}?'.format(pred_tuple[0].lower(), pred_tuple[4])

follow_up_filler = lambda pred_tuple: 'How likely is it that {} {}?'.format(pred_tuple[2].lower(), pred_tuple[4])

item_temp = lambda typ, first, second: Template('''[$typ, "AcceptabilityJudgment", {s: "$first", q: "$second"}]''').substitute(typ=typ, first=first, second=second)
...
def create_debrief_controller(size=15):
    ...
    return '[["debrief", 0], "Message", {html: "<p><center>Please enter the following code into Mechanical Turk.</center></p><p><center><b>'+code+'</b></center></p>", transfer: null}]'
...
experiment = Template('''
var shuffleSequence = seq("consent", "setcounter", "intro", sepWith("sep", "practice"), "begin", sepWith("sep", rshuffle(shuffle($test_labels), shuffle($filler_labels))), "sr", "debrief");
